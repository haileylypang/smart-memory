...
function App(){
 ...
 useEffect(()=>{winSound.current=new Audio(`${import.meta.env.BASE_URL}audio/win.mp3`);loseSound.current=new Audio(`${import.meta.env.BASE_URL}audio/lose.mp3`);winSound.current.volume=.25;loseSound.current.volume=.03},[]);
 useEffect(()=>()=>{run.current++},[]);
 const tone=(i,d=240)=>{if(muted)return; const C=window.AudioContext||window.webkitAudioContext; audio.current ||= new C(); const ctx=audio.current,o=ctx.createOscillator(),g=ctx.createGain(); o.type='sine';o.frequency.value=NOTES[i];g.gain.setValueAtTime(.0001,ctx.currentTime);g.gain.exponentialRampToValueAtTime(1,ctx.currentTime+.02);g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+d/1000);o.connect(g).connect(ctx.destination);o.start();o.stop(ctx.currentTime+d/1000+.03)};
